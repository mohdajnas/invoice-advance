class InvoiceApp {
    constructor() {
        ...
        // NEW CURRENCY PROPERTIES
        this.currentCurrency = 'INR'; // Default currency
    ...
    // ADD NEW METHOD: Initialize currency controls
    initializeCurrency() {
        ...
        if (currencySelect) {
            ...
            currencyDisplays.forEach(display => {
                display.textContent = this.currencySymbols[this.currentCurrency];
            });
        }
    }
    ...
    attachEventListeners() {
        ...
        const currencySelect = document.getElementById('currencySelect');
        if (currencySelect) {
            currencySelect.addEventListener('change', (e) => this.updateCurrency(e.target.value));
        }
    ...
    // ADD NEW METHOD: Update currency
    updateCurrency(newCurrency) {
        this.currentCurrency = newCurrency;
        
        // Update all currency displays in the interface
        const currencyDisplays = document.querySelectorAll('.currency-display');
        currencyDisplays.forEach(display => {
            display.textContent = this.currencySymbols[newCurrency];
        });
        
        // Update the currency selector display value
        const currencyDisplayElement = document.getElementById('currencyDisplay');
        if (currencyDisplayElement) {
            currencyDisplayElement.textContent = this.currencySymbols[newCurrency];
        }
        
        // Recalculate and update all amounts with new currency
        this.updateCalculations();
        this.updateViewTexts();
        
        console.log(`Currency changed to: ${newCurrency} (${this.currencySymbols[newCurrency]})`);
    }
    ...
    saveData() {
        const data = {
            ...
            currentCurrency: this.currentCurrency, // NEW LINE
            items: []
        };
    ...
    loadData(data) {
        ...
        // Load currency settings - NEW SECTION
        if (data.currentCurrency) {
            this.currentCurrency = data.currentCurrency;
            const currencySelect = document.getElementById('currencySelect');
            if (currencySelect) {
                currencySelect.value = data.currentCurrency;
            }
            this.updateCurrency(data.currentCurrency);
        }
